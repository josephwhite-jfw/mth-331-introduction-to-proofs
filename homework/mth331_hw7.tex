...
Here is a standard example on $\mathbb{N} = \{0,1,2,3,\dots\}$. Every natural number $n \ge 1$ can be written uniquely as
\[
n = 2^k m,
\]
where $k \ge 0$ and $m$ is odd. For each $k \ge 0$ define
\[
S_k = \{ n \in \mathbb{N} : n > 0 \text{ and } n = 2^k m \text{ for some odd } m \}.
\]
Finally, to incorporate $0$, we simply include it in one of the sets, for example by redefining
\[
S_0 := S_0 \cup \{0\}.
\]
...
So, as a single set,
\[
R_P = \{(0,0), (1,1), (1,2), (2,1), (2,2), (3,3), (3,4), (3,5), (4,3), (4,4), (4,5), (5,3), (5,4), (5,5)\}.
\]

Two elements of $A$ are related by $R_P$ exactly when they lie in the same part of the partition $P$. For instance, $1$ and $2$ are related, and $3$ and $5$ are related, but $0$ and $1$ are not, and $2$ and $3$ are not.
